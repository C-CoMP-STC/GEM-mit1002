...
final_results = find_reciprocal_best_hits(
    query_seq_file=os.path.join(PROJECT_ROOT, "BLAST/query_ecoli_dut/dut_seq.fa"),
    organism_fasta=os.path.join(
        PROJECT_ROOT, "genome/Michelle_4106_gene_calls/MIT1002_anvio_prot_seqs.fa"
    ),
    organism_db=os.path.join(PROJECT_ROOT, "BLAST/dbs/amac_db"),
    reference_db=os.path.join(PROJECT_ROOT, "BLAST/dbs/ecoli_db"),
    output_dir=os.path.join(PROJECT_ROOT, "BLAST/query_ecoli_dut/results"),
    num_forward_hits=10,
)
